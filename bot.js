    ...
    bot.on('message', msg => {
        ...
        for (var i = 0; i < age.length; i++) {
            if (msg.content.toLowerCase()
                .includes(age[i]) && msg.content.includes(minornumber[i])) {
            ...
            for (var i = 0; i < age.length; i++) {
                if (msg.content.toLowerCase()
                    .includes(age[i]) && msg.content.includes(adultnumber[i])) {
        ...
        for (var i = 0; i < age.length; i++) {
            if (msg.content.toLowerCase()
                .includes(age[i]) && msg.content.includes(youngadultnumber[i])) {
        ...
        for (var i = 0; i < explicit.length; i++) {
            fs.readFile(`db/${msg.author.id}`, 'utf8', function (err, data) {
                ...
                if (msg.content.toLowerCase()
                    .includes(explicit[i]) && userage == "Adult") {
                    msg.channel.send({
                        embed: {
                            color: 'e74c3c',
                            author: {
                ...
                if (msg.content.toLowerCase()
                    .includes(explicit[i]) && userage == "Pedophile") {
                    msg.channel.send({
                        embed: {
                            color: 'e74c3c',
                            author: {
