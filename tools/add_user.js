...
function userAdd(username, suspicious, pedophile) {
	db.get("SELECT * FROM users WHERE Username = ?", [username], function(err, result) {
		if (err) main.log(err);
		if (result === undefined) {
			db.run(`INSERT INTO users (Username, Suspicious, Pedophile, Modified) VALUES('${username}', '${suspicious}', '${pedophile}', ${main.date()})`, function(err) {
					if (err) main.log(err);
			...
			db.run(`INSERT INTO users (ID, Suspicious, Pedophile, Age, Modified) VALUES('${UserID}', '${suspicious}', '${pedophile}', ${UserAge}, ${main.date()}) ON CONFLICT(ID) DO UPDATE SET Suspicious = '${suspicious}', Pedophile = '${pedophile}', Modified = '${main.date()}'`, function(err) {
				if (err) main.log(err);
			});
		}
	});
